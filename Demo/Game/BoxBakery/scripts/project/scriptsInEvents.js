function filterItems(data, conditionFunction) {
  const items = typeof data === 'string' ? JSON.parse(data) : data;
  return items.filter(item => conditionFunction(item));
}
...
const scriptsInEvents = {
	...
	async Main_Event56_Act1(runtime, localVars)
	{
		// 获取 CookFormula 数据
	...
	async Data_Event17_Act1(runtime, localVars)
	{
	...
	async Data_Event18_Act1(runtime, localVars)
	{
