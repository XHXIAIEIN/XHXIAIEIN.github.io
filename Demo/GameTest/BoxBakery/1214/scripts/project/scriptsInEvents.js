...
const scriptsInEvents = {
	...
	async Data_Event33_Act1(runtime, localVars)
	{
		// 获取 CookFormula 数据
		...
		// 创建配料计数映射
		const ingredientCounts = Object.entries(parsedFormula.Ingredient).reduce((counts, [ingredient, quantity]) => {
		...
		const matched = cookFormula.filter(recipe => {
		    // 检查食谱中的配料是否匹配
		    const ingredientsMatch = recipe.Ingredients.every(ingredient => ingredientCounts[ingredient] === (ingredientCounts[ingredient] || 0));
		...
		// 提取匹配到的食谱名称数组
		const matchedRecipeNames = matched.map(recipe => recipe.ItemID);
